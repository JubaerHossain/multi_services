...
class SettingController extends Controller
{
    public function color(){
        ...
        return view('Backend.settings.color',compact('data'));
    }

    function ColorStore($id){
        $data = Color::find($id);
        if ($data) {
           $seeting = Settings::first();
           $seeting->color = $id;
           $seeting->save();
    ...
    function general_setting(){
        ...
        $data['color'] = Color::pluck('name','id');
        $data['tax'] = Tax::pluck('name','id');
        $data['currency'] = Currency::pluck('name','id');
        $data['barcode'] = Barcode::pluck('name','name');
    ...
    public function UpdateGeneral(Request $request)
    {
        $this->validate($request, [
            ...
            'file' => 'nullable|mimes:jpeg,png,jpg|dimensions:max_width=60,max_height=60',
            'favicon' => 'nullable|mimes:jpeg,png,jpg|dimensions:max_width=32,max_height=32',
        ]);
        try {
