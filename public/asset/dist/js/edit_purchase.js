...
purchaseSearch = (str) => {
    ...
    $.ajax({
            method:'GET',
            url: url + '/' + 'purchases/product_search',
            data: {
                data: str
            },
...
function calculateRowProductData(quantity) {
    ...
    $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.tax-rate').val(tax_rate[rowindex].toFixed(2));
        ...
        var net_unit_cost = product_discount[rowindex] * quantity;
        
        var tax = row_product_cost * quantity * (tax_rate[rowindex] / 100);
        var sub_total = (row_product_cost * quantity - net_unit_cost) + tax;

        $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('td:nth-child(4)').text(row_product_cost);
        $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.net_unit_cost').val(row_product_cost);
...
$("#myTable").on('input', '.qty', function() {
    rowindex = $(this).closest('tr').index();
    row_product_cost = $('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.net_unit_cost').text(); 
...
submitHandler: function(form) {
    ...
    if (!$("select[name='supplier']").val()){
        $("#supplier-error").text('This field is required.')
...
$('select[name="payment_method"]').on("change", function() {
    if ($("select[name='payment_method']").val() ==1) {
        ...
        $(".date_pick").removeClass("col-lg-12")
        $(".date_pick").addClass("col-lg-6")
    }else{
        $(".bank_select").addClass("d-none")
        $(".date_pick").addClass("col-lg-12")
        $(".date_pick").removeClass("col-lg-6")
    }
});
